...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-[#f6f1e8] text-zinc-900 antialiased">
        ...
        <header class="sticky top-0 z-20 border-b border-black/5 bg-white/75 backdrop-blur-xl">
            <div class="mx-auto flex max-w-6xl items-center justify-between gap-4 py-4 sm:px-6 lg:px-8">
                <a href="{{ route('home') }}" class="flex items-center gap-5" wire:navigate>
                    <img
                        src="{{ asset('images/distrack_logo.png') }}"
                        alt="{{ config('app.name') }}"
                        class="h-14 w-auto shrink-0 object-contain sm:h-20 lg:h-24"
                        decoding="async"
                    />
                    <div>
