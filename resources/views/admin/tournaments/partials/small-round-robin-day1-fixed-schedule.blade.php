@php
    $day1DefaultScheduleRowCount = count($smallDay1FormDefaults);
    $day1ScheduleFormRowCount = $day1ScheduleFormRowCount ?? $smallDayOneGridRows->count();
    $day1ScheduleRowErrors = collect($errors->getMessages())->filter(fn ($_, $k) => str_starts_with((string) $k, 'day1_rows.'));
    $day1SlotRowErrors = collect($errors->getMessages())->filter(fn ($_, $k) => str_starts_with((string) $k, 'day1_slot.'));
    $day1ScheduleUnsynced = $day1ScheduleUnsynced ?? false;
    ...
    $smallDay1FormDefaults = $smallDay1FormDefaults ?? [];
    $removedDayOneScheduleGroups = $removedDayOneScheduleGroups ?? collect();
...
<section class="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
    <div class="space-y-6 p-6 sm:p-8">
        @if ($day1SlotRowErrors->isNotEmpty())
            <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/60 dark:bg-red-950/40 dark:text-red-300">
                <p class="font-semibold">{{ __('Fix the Day 1 add/edit schedule fields, then try again.') }}</p>
                <ul class="mt-2 list-inside list-disc space-y-0.5">
                    @foreach ($day1SlotRowErrors as $messages)
        ...
        @if ($errors->has('small_day1_schedule'))
            <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/60 dark:bg-red-950/40 dark:text-red-300">
                {{ $errors->first('small_day1_schedule') }}
            </div>
        @endif
        ...
        @if ($day1ScheduleRowErrors->isNotEmpty())
            <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/60 dark:bg-red-950/40 dark:text-red-300">
                <p class="font-semibold">{{ __('Fix the Day 1 schedule fields below, then sync again.') }}</p>
                <ul class="mt-2 list-inside list-disc space-y-0.5">
                    @foreach ($day1ScheduleRowErrors as $messages)
                        @foreach ($messages as $message)
                            <li>{{ $message }}</li>
                        @endforeach
                    @endforeach
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            ...
            <div class="rounded-xl border border-neutral-200 bg-zinc-50 px-4 py-3 dark:border-neutral-700 dark:bg-zinc-950">
                ...
                <div class="mt-1 text-[11px] text-zinc-500 dark:text-zinc-400">{{ __('Two games per row; pick any tournament pitch per game (defaults follow sort order).') }}</div>
            </div>
            <div class="rounded-xl border border-neutral-200 bg-zinc-50 px-4 py-3 dark:border-neutral-700 dark:bg-zinc-950">
                <div class="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-500 dark:text-zinc-400">{{ __('Tracked Day 1 games') }}</div>
                <div class="mt-2 text-2xl font-semibold text-zinc-900 dark:text-white">
                    {{ $selectedTournament->matches->filter(fn ($m) => $m->stage === 'round_robin' && \App\Support\SmallFixedRoundRobinDayOneSchedule::isTrackedMatch($m))->count() }}
                </div>
            </div>
        </div>
        ...
        @if ($day1ScheduleUnsynced && $isAdmin)
            <form
                id="small-day1-schedule-sync-form"
                method="POST"
                action="{{ route('admin.tournaments.matches.small-day1-schedule.sync', $selectedTournament) }}"
                class="space-y-6"
                onsubmit="return (typeof window.confirmSmallDay1ScheduleSyncSubmit === 'function' ? window.confirmSmallDay1ScheduleSyncSubmit(this) : true)"
            >
                @csrf
                <input type="hidden" name="small_day1_sync_from_grid" value="1">
                @if ($day2AttachToDay1Form ?? false)
        ...
        <div class="overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">
            <div class="border-b border-neutral-200 bg-gradient-to-br from-zinc-50 to-white px-6 py-6 text-center dark:border-neutral-700 dark:from-zinc-900 dark:to-zinc-950">
                ...
                <h2 class="mt-3 text-3xl font-bold uppercase tracking-[0.06em] text-zinc-900 dark:text-white">{{ __('DAY 1') }}</h2>
                <p class="mt-2 text-lg font-semibold text-zinc-700 dark:text-zinc-200">{{ \App\Support\SmallFixedRoundRobinDayOneSchedule::SCHEDULE_DATE_LABEL }}</p>
            </div>
            ...
            @if ($isAdmin)
                <div class="flex flex-wrap items-center gap-2 border-b border-neutral-200 bg-zinc-50/80 px-4 py-3 dark:border-neutral-700 dark:bg-zinc-900/50">
                    @if ($day1ScheduleUnsynced)
                        <flux:button type="button" variant="outline" size="sm" onclick="window.addSmallDay1ScheduleRow?.()">
                            {{ __('+ Add Schedule') }}
                        </flux:button>
                    @else
                        <flux:modal.trigger name="small-day1-add-schedule-slot-modal">
            ...
            <div class="overflow-x-auto">
                <table class="{{ $day1ScheduleUnsynced && $isAdmin ? 'min-w-[100rem]' : 'min-w-[60rem]' }} w-full text-left text-sm">
                ...
                @for ($i = 0; $i < $day1ScheduleFormRowCount; $i++)
                    @php
                        $row = $smallDayOneGridRows->get($i) ?? [
                            'time_label' => '—',
                            'round' => $i + 1,
                            'pitch1_game_no' => $i * 2 + 1,
                            'pitch2_game_no' => $i * 2 + 2,
                            'pitch1_matchup' => '—',
                            'pitch2_matchup' => '—',
                            'pitch1_match' => null,
                            'pitch2_match' => null,
                        ];
                        $defaults = $smallDay1FormDefaults[$i] ?? [
                            'round' => $i + 1,
                            'start_time' => '07:00',
                            'end_time' => '07:40',
                            ...
                            'status' => 'upcoming',
                            'match1_match_number' => $i * 2 + 1,
                            'match2_match_number' => $i * 2 + 2,
                        ];
                        $p1 = $row['pitch1_match'];
                        $p2 = $row['pitch2_match'];
                        ...
                        $day1SyncedRowRemovable = false;
                        if (! $day1ScheduleUnsynced && $isAdmin && $p1 && $p2) {
                            $day1SyncedRowRemovable = true;
                            foreach ([$p1, $p2] as $m) {
                    ...
                    <tr
                        ...
                        data-day1-row-client-id="{{ (string) str()->uuid() }}"
                        data-day1-schedule-row-source="{{ $i < $day1DefaultScheduleRowCount ? 'default' : 'added' }}"
                    >
                        @if ($day1ScheduleUnsynced && $isAdmin)
                            <td class="whitespace-nowrap px-2 py-2 align-top">
                                <input
                                    type="time"
                                    name="day1_rows[{{ $i }}][start_time]"
                                    value="{{ old('day1_rows.'.$i.'.start_time', $defaults['start_time']) }}"
                                    required
                                    class="{{ $inputClass }}"
                                >
                            </td>
                            <td class="whitespace-nowrap px-2 py-2 align-top">
                                <input
                                    type="time"
                                    name="day1_rows[{{ $i }}][end_time]"
                                    value="{{ old('day1_rows.'.$i.'.end_time', $defaults['end_time']) }}"
                                    required
                                    class="{{ $inputClass }}"
                                >
                            </td>
                            <td class="whitespace-nowrap px-2 py-2 align-top">
                                <input
                                    type="number"
                                    name="day1_rows[{{ $i }}][round]"
                                    min="1"
                                    required
                                    value="{{ old('day1_rows.'.$i.'.round', $defaults['round']) }}"
                            ...
                            <td class="whitespace-nowrap px-2 py-2 align-top">
                                <input
                                    type="number"
                                    name="day1_rows[{{ $i }}][match1_match_number]"
                                    min="1"
                                    required
                                    value="{{ old('day1_rows.'.$i.'.match1_match_number', $defaults['match1_match_number'] ?? $row['pitch1_game_no']) }}"
                                    class="{{ $inputClass }} w-16"
                                >
                            </td>
                            <td class="min-w-[6rem] px-2 py-2 align-top">
                                <select name="day1_rows[{{ $i }}][pitch1_pitch_id]" required class="{{ $inputClass }}">
                                    @foreach ($roundRobinPitchOptions as $pitchOpt)
                                        <option
                                            value="{{ $pitchOpt->id }}"
                                            @selected((int) old('day1_rows.'.$i.'.pitch1_pitch_id', (int) ($defaults['pitch1_pitch_id'] ?? 0)) === (int) $pitchOpt->id)
                                        >{{ $pitchOpt->name }}</option>
                                    @endforeach
                                </select>
                                @error('day1_rows.'.$i.'.pitch1_pitch_id')
                                    <p class="mt-0.5 text-[11px] text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </td>
                            <td class="min-w-[7rem] px-2 py-2 align-top">
                                <select name="day1_rows[{{ $i }}][match1_home_registration_id]" required class="{{ $inputClass }}">
                                    @foreach ($roundRobinRegistrationOptions as $reg)
                                        <option
                                            value="{{ $reg->id }}"
                                            @selected((int) old('day1_rows.'.$i.'.match1_home_registration_id', $defaults['match1_home_registration_id'] ?? 0) === (int) $reg->id)
                                        >{{ $regLabel($reg) }}</option>
                                    @endforeach
                                </select>
                                @error('day1_rows.'.$i.'.match1_home_registration_id')
                                    <p class="mt-0.5 text-[11px] text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </td>
                            <td class="min-w-[7rem] px-2 py-2 align-top">
                                <select name="day1_rows[{{ $i }}][match1_away_registration_id]" required class="{{ $inputClass }}">
                                    @foreach ($roundRobinRegistrationOptions as $reg)
                                        <option
                                            value="{{ $reg->id }}"
                                            @selected((int) old('day1_rows.'.$i.'.match1_away_registration_id', $defaults['match1_away_registration_id'] ?? 0) === (int) $reg->id)
                                        >{{ $regLabel($reg) }}</option>
                                    @endforeach
                                </select>
                                @error('day1_rows.'.$i.'.match1_away_registration_id')
                                    <p class="mt-0.5 text-[11px] text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </td>
                            <td class="whitespace-nowrap px-2 py-2 align-top">
                                <input
                                    type="number"
                                    name="day1_rows[{{ $i }}][match2_match_number]"
                                    min="1"
                                    required
                                    value="{{ old('day1_rows.'.$i.'.match2_match_number', $defaults['match2_match_number'] ?? $row['pitch2_game_no']) }}"
                                    class="{{ $inputClass }} w-16"
                                >
                            </td>
                            <td class="min-w-[6rem] px-2 py-2 align-top">
                                <select name="day1_rows[{{ $i }}][pitch2_pitch_id]" required class="{{ $inputClass }}">
                                    @foreach ($roundRobinPitchOptions as $pitchOpt)
                                        <option
                                            value="{{ $pitchOpt->id }}"
                                            @selected((int) old('day1_rows.'.$i.'.pitch2_pitch_id', (int) ($defaults['pitch2_pitch_id'] ?? 0)) === (int) $pitchOpt->id)
                                        >{{ $pitchOpt->name }}</option>
                                    @endforeach
                                </select>
                                @error('day1_rows.'.$i.'.pitch2_pitch_id')
                                    <p class="mt-0.5 text-[11px] text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </td>
                            <td class="min-w-[7rem] px-2 py-2 align-top">
                                <select name="day1_rows[{{ $i }}][match2_home_registration_id]" required class="{{ $inputClass }}">
                                    @foreach ($roundRobinRegistrationOptions as $reg)
                                        <option
                                            value="{{ $reg->id }}"
                                            @selected((int) old('day1_rows.'.$i.'.match2_home_registration_id', $defaults['match2_home_registration_id'] ?? 0) === (int) $reg->id)
                                        >{{ $regLabel($reg) }}</option>
                                    @endforeach
                                </select>
                                @error('day1_rows.'.$i.'.match2_home_registration_id')
                                    <p class="mt-0.5 text-[11px] text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </td>
                            <td class="min-w-[7rem] px-2 py-2 align-top">
                                <select name="day1_rows[{{ $i }}][match2_away_registration_id]" required class="{{ $inputClass }}">
                                    @foreach ($roundRobinRegistrationOptions as $reg)
                                        <option
                                            value="{{ $reg->id }}"
                                            @selected((int) old('day1_rows.'.$i.'.match2_away_registration_id', $defaults['match2_away_registration_id'] ?? 0) === (int) $reg->id)
                                        >{{ $regLabel($reg) }}</option>
                                    @endforeach
                                </select>
                                @error('day1_rows.'.$i.'.match2_away_registration_id')
                                    <p class="mt-0.5 text-[11px] text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </td>
                            <td class="max-w-[8rem] px-2 py-2 align-top">
                                <select name="day1_rows[{{ $i }}][status]" class="{{ $inputClass }}">
                                    @foreach ($smallDayOneRowStatusOptions as $value => $label)
                                        <option value="{{ $value }}" @selected(old('day1_rows.'.$i.'.status', $defaults['status'] ?? 'upcoming') === $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td class="whitespace-nowrap px-2 py-2 align-top">
                                <button
                                    ...
                                    data-day1-remove-row="1"
                                    onclick="window.removeSmallDay1ScheduleRow?.(this)"
                                >−</button>
                            </td>
                        @else
                            ...
                            <td class="max-w-[14rem] px-4 py-2.5 align-top">
                                @if ($p1 || $p2)
                                    <div class="flex flex-col gap-1.5">
                                        <form
                                            method="POST"
                                            action="{{ route('admin.tournaments.matches.small-day1-slot-status.update', $selectedTournament) }}"
                                        ...
                                        >
                                            ...
                                            <input type="hidden" name="redirect_route" value="admin.tournaments.index">
                                            <input type="hidden" name="redirect_tab" value="round-robin">
                                            <input type="hidden" name="first_game_number" value="{{ $row['pitch1_game_no'] }}">
                            ...
                            <td class="whitespace-nowrap px-2 py-2 align-top">
                                @if ($isAdmin && $p1 && $p2)
                                    <div class="flex flex-col gap-1">
                                        @if ($day1SyncedRowRemovable)
                                            <form
                                                method="POST"
                                                action="{{ route('admin.tournaments.round-robin.schedule-row.destroy', $selectedTournament) }}"
                                                class="inline"
                                        ...
                                        @else
                                            ...
                                            >−</button>
                                        @endif
                                        <flux:modal.trigger name="small-day1-edit-schedule-slot-{{ $p1->id }}-{{ $p2->id }}">
                ...
                @if (! $day1ScheduleUnsynced && $isAdmin)
                    @php
                        $day1Tz = \App\Support\SmallFixedRoundRobinDayOneSchedule::tournamentTimezone($selectedTournament);
                        $day1AddMaxGame = 0;
                        $day1AddMaxRound = 0;
                        foreach ($smallDayOneGridRows as $__gr) {
                            $day1AddMaxGame = max($day1AddMaxGame, (int) ($__gr['pitch1_game_no'] ?? 0), (int) ($__gr['pitch2_game_no'] ?? 0));
                            $day1AddMaxRound = max($day1AddMaxRound, (int) ($__gr['round'] ?? 0));
                        }
                        $day1AddDefaults = [
                            'round' => $day1AddMaxRound + 1,
                            'start_time' => '07:00',
                            'end_time' => '07:40',
                            'match1_match_number' => $day1AddMaxGame + 1,
                            'match2_match_number' => $day1AddMaxGame + 2,
                    ...
                    @include('admin.tournaments.partials.small-round-robin-slot-row-admin-modal', [
                        ...
                        'modalKey' => 'add',
                        'slotPrefix' => 'day1_slot',
                        'modalName' => 'small-day1-add-schedule-slot-modal',
                        'heading' => __('Add Day 1 schedule row'),
                        'formAction' => route('admin.tournaments.round-robin.day1-schedule-rows.store', $selectedTournament),
                        'httpMethod' => 'post',
                        'dateLabel' => \App\Support\SmallFixedRoundRobinDayOneSchedule::SCHEDULE_DATE_LABEL,
                        'slotDateIso' => \App\Support\SmallFixedRoundRobinDayOneSchedule::SCHEDULE_DATE_ISO,
                        'pitch1Match' => null,
                        'pitch2Match' => null,
                        'defaults' => $day1AddDefaults,
                    ...
                    @for ($mi = 0; $mi < $day1ScheduleFormRowCount; $mi++)
                        @php
                            $erow = $smallDayOneGridRows->get($mi) ?? [];
                        ...
                        @if ($ep1 && $ep2)
                            @php
                                ...
                                $slotEditDefaults = [
                                    ...
                                    'start_time' => $ep1->scheduled_at?->timezone($day1Tz)->format('H:i') ?? '07:00',
                                    'end_time' => $ep1->scheduled_ends_at?->timezone($day1Tz)->format('H:i') ?? '07:40',
                            ...
                            @include('admin.tournaments.partials.small-round-robin-slot-row-admin-modal', [
                                ...
                                'slotPrefix' => 'day1_slot',
                                'modalName' => 'small-day1-edit-schedule-slot-'.$ep1->id.'-'.$ep2->id,
                                'heading' => __('Edit Day 1 schedule row'),
                                'formAction' => route('admin.tournaments.round-robin.day1-schedule-rows.update', $selectedTournament),
                                'httpMethod' => 'patch',
                                'dateLabel' => \App\Support\SmallFixedRoundRobinDayOneSchedule::SCHEDULE_DATE_LABEL,
                                'slotDateIso' => \App\Support\SmallFixedRoundRobinDayOneSchedule::SCHEDULE_DATE_ISO,
                                'pitch1Match' => $ep1,
                ...
                @if (! $day1ScheduleUnsynced && $isAdmin && $removedDayOneScheduleGroups->isNotEmpty())
                    <div class="border-t border-neutral-200 bg-zinc-50/90 px-4 py-4 dark:border-neutral-700 dark:bg-zinc-900/60">
                        <h3 class="text-sm font-semibold text-zinc-900 dark:text-white">{{ __('Removed schedule rows (Day 1)') }}</h3>
                        ...
                        <div class="mt-3 overflow-x-auto">
                            <table class="min-w-[48rem] w-full text-left text-xs">
                                ...
                                <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                                    @foreach ($removedDayOneScheduleGroups as $removedRow)
                                        <tr>
                                            ...
                                            <td class="py-2">
                                                <form method="POST" action="{{ route('admin.tournaments.round-robin.day1-schedule-rows.restore', $selectedTournament) }}" class="inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="redirect_route" value="admin.tournaments.index">
                                                    <input type="hidden" name="redirect_tab" value="round-robin">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div class="min-w-0">
                <p class="text-sm font-medium text-zinc-900 dark:text-white">{{ __('Fixed Day 1 schedule') }}</p>
                <p class="mt-2 max-w-3xl text-sm text-zinc-600 dark:text-zinc-300">
                    @if ($day1ScheduleUnsynced && $isAdmin)
                        {{ __('Adjust times, pitches, matchups (home/away from registered teams), and row status before syncing. The day date above applies to every row. Times apply to both games in the row. Sync writes all rows to the matches table.') }}
                    @else
                        {{ __('Twelve time slots with two games each (24 games). Pairings follow Berger round-robin order using teams sorted by seed. Sync writes rows to the matches table so you can edit status and scores like other round robin games.') }}
                    @endif
                </p>
                ...
                @if ($day1ScheduleUnsynced && $isAdmin && ($day2AttachToDay1Form ?? false))
                    <p class="mt-2 text-xs font-medium text-zinc-600 dark:text-zinc-300">
                        {{ __('Day 2 schedule edits on this page are included when you submit the button below.') }}
                    </p>
                @endif
            </div>
            ...
            @if ($isAdmin)
                @if ($day1ScheduleUnsynced)
                    <div class="flex shrink-0 flex-col items-stretch gap-2 sm:flex-row sm:items-center">
                        @if ($isAdmin)
                            <flux:button type="button" variant="ghost" onclick="window.resetSmallDay1ScheduleForm?.()">
                                {{ __('Reset to Default Schedule') }}
                            </flux:button>
                            <flux:button type="submit" variant="primary" :disabled="$teamCount === 0">
                                @if ($day2AttachToDay1Form ?? false)
                                    {{ __('Sync Day 1 & Day 2 schedules to database') }}
                                @else
                                    {{ __('Sync Day 1 schedule to database') }}
                                @endif
                            </flux:button>
                        @endif
                    </div>
                @else
                    <form method="POST" action="{{ route('admin.tournaments.matches.small-day1-schedule.sync', $selectedTournament) }}" class="shrink-0">
                        @csrf
                        <input type="hidden" name="redirect_route" value="admin.tournaments.index">
                        <input type="hidden" name="redirect_tab" value="round-robin">
                        <flux:button type="submit" variant="primary" :disabled="$teamCount === 0">
        ...
        @if ($day1ScheduleUnsynced && $isAdmin)
            ...
            <script>
                window.__SMALL_DAY1_DEFAULTS = @json($smallDay1FormDefaults);
                window.__SMALL_DAY1_DEFAULT_ROW_COUNT = @json($day1DefaultScheduleRowCount);
                window.__SMALL_DAY1_ROW_TEMPLATE = (function () {
                    const tbody = document.querySelector('#small-day1-schedule-sync-form tbody');

                    return tbody ? tbody.querySelector('tr')?.cloneNode(true) ?? null : null;
                })();
                window.__DAY1_REMOVE_TITLE = @json(__('Remove schedule row (both games)'));
                window.__DAY1_SYNC_NEED_ROW_ALERT = @json(__('Add at least one schedule row before syncing.'));
                window.__SMALL_DAY2_ATTACH = @json((bool) ($day2AttachToDay1Form ?? false));
                window.__DAY2_SYNC_NEED_ROW_ALERT = @json(__('Add at least one Day 2 schedule row before syncing.'));
                window.confirmSmallDay1ScheduleSyncSubmit = function (form) {
                    if (form.id !== 'small-day1-schedule-sync-form') {
                        return true;
                    }
                    const tbody = form.querySelector('tbody');
                    if (!tbody || tbody.querySelectorAll('tr').length === 0) {
                        window.alert(window.__DAY1_SYNC_NEED_ROW_ALERT);

                        return false;
                    }
                    if (window.__SMALL_DAY2_ATTACH) {
                        const d2 = document.querySelector('#small-day2-schedule-tbody');
                        if (!d2 || d2.querySelectorAll('tr').length === 0) {
                ...
                window.refreshDay1UnsyncedRemoveButtons = function () {
                    const tbody = document.querySelector('#small-day1-schedule-sync-form tbody');
                    if (!tbody) return;
                    tbody.querySelectorAll('[data-day1-remove-row]').forEach(function (btn) {
                        btn.disabled = false;
                        btn.setAttribute('title', window.__DAY1_REMOVE_TITLE);
                    });
                };
                window.reindexSmallDay1ScheduleRows = function () {
                    const tbody = document.querySelector('#small-day1-schedule-sync-form tbody');
                    if (!tbody) return;
                    tbody.querySelectorAll('tr').forEach(function (tr, idx) {
                        tr.querySelectorAll('[name^="day1_rows["]').forEach(function (el) {
                            el.name = el.name.replace(/day1_rows\[\d+]/, 'day1_rows[' + idx + ']');
                        });
                    });
                    window.refreshDay1UnsyncedRemoveButtons();
                };
                window.removeSmallDay1ScheduleRow = function (btn) {
                    const tr = btn.closest('tr');
                    const tbody = tr && tr.parentElement;
                    if (!tbody) return;
                    tr.remove();
                    window.reindexSmallDay1ScheduleRows();
                };
                window.addSmallDay1ScheduleRow = function () {
                    const form = document.querySelector('#small-day1-schedule-sync-form');
                    if (!form) return;
                    const tbody = form.querySelector('tbody');
                    if (!tbody) return;
                    const trs = tbody.querySelectorAll('tr');
                    const tmpl = window.__SMALL_DAY1_ROW_TEMPLATE;
                    ...
                    clone.setAttribute('data-day1-schedule-row-source', 'added');
                    clone.setAttribute('data-day1-row-client-id', window.crypto?.randomUUID?.() || String(Date.now()) + '-' + Math.random());
                ...
                window.resetSmallDay1ScheduleForm = function () {
                    const tbody = document.querySelector('#small-day1-schedule-sync-form tbody');
                    const tmpl = window.__SMALL_DAY1_ROW_TEMPLATE;
                    if (!tbody || !tmpl) return;
                    const n = window.__SMALL_DAY1_DEFAULT_ROW_COUNT || 12;
                    tbody.innerHTML = '';
                    for (let i = 0; i < n; i++) {
                        const tr = tmpl.cloneNode(true);
                        tr.setAttribute('data-day1-schedule-row-source', 'default');
                        tr.setAttribute('data-day1-row-client-id', window.crypto?.randomUUID?.() || String(Date.now()) + '-' + i + '-' + Math.random());
                        tbody.appendChild(tr);
                    }
                    window.reindexSmallDay1ScheduleRows();
                    const rows = window.__SMALL_DAY1_DEFAULTS;
                    if (!Array.isArray(rows)) return;
                    rows.forEach(function (row, i) {
                        Object.keys(row).forEach(function (key) {
                            const el = document.querySelector(
                                '#small-day1-schedule-sync-form [name="day1_rows[' + i + '][' + key + ']"]'
                            );
                            if (el && 'value' in el) {
...
@if ($smallDayOneOtherRobinMatches->isNotEmpty())
    <section class="rounded-xl border border-amber-200 bg-amber-50/70 px-4 py-4 dark:border-amber-900/70 dark:bg-amber-950/20">
        <div class="text-sm font-semibold text-amber-900 dark:text-amber-100">{{ __('Additional round robin matches') }}</div>
        <p class="mt-1 text-xs text-amber-800 dark:text-amber-200">
            {{ __('These games are outside the fixed Day 1 grid (they were not created by the Day 1 sync). Manage them from the scoring console or edit each match below.') }}
        </p>
        <ul class="mt-2 list-inside list-disc text-xs text-amber-900 dark:text-amber-100">
            @foreach ($smallDayOneOtherRobinMatches as $otherMatch)
                <li>
                    {{ __('Game :num — :home vs :away', [
                        'num' => $otherMatch->match_number ?? '—',
                        'home' => $otherMatch->homeRegistration?->team?->name ?? __('TBD'),
                        'away' => $otherMatch->awayRegistration?->team?->name ?? __('TBD'),
                    ]) }}
                    @if ($isAdmin)
                        <flux:modal.trigger name="setup-edit-round-robin-match-modal-{{ $otherMatch->id }}">
                            <button type="button" class="ml-1 font-semibold text-amber-900 underline dark:text-amber-50">{{ __('Edit') }}</button>
                        </flux:modal.trigger>
                    @endif
                </li>
            @endforeach
